...
def minibatch_KLD_upper_bound(covar_module0, covar_module1, likelihood, latent_dim, m, H, train_xt, mu, log_v, z, P_tot, P_batch, T, natural_gradient, eps):
    ...
    try:
        ...
        print('Cholesky decomposition failed. Trying to add jitter...')
        K0zz = K0zz + 1e-6 * torch.eye(M, dtype=torch.double).to(device)
        LK0zz = torch.linalg.cholesky(K0zz)
...
def minibatch_KLD_upper_bound_iter(covar_module0, covar_module1, likelihood, latent_dim, m, H, train_xt, mu, log_v, z, P, P_in_current_batch, N, natural_gradient, id_covariate, eps):
    ...
    # print(f"MINIBARCH DEVICE: {device}")
    ...
    for s in subjects:
        ...
        try:
            ...
            print('Cholesky decomposition failed. Trying to add jitter...')
            is_symmetric = torch.allclose(B_st, B_st.T)
            print("B_st is symmetric:", is_symmetric)
            eigenvalues = torch.eig(B_st, eigenvectors=False)[0][:, 0]  # get the real parts of eigenvalues
            is_positive_definite = torch.all(eigenvalues > 0)
            print("B_st is positive-definite:", is_positive_definite)
            B_st = B_st + 1e-6 * torch.eye(T, dtype=torch.double).to(device)
            LB_st = torch.linalg.cholesky(B_st)
        iB_st = torch.cholesky_solve(torch.eye(T, dtype=torch.double).to(device), LB_st).squeeze(dim=0)
