...
from utils import SubjectSampler, VaryingLengthSubjectSampler, VaryingLengthBatchSampler, HensmanDataLoader, \
    train_COX_on_latent, plot_training_values, plot_validation_values, calculate_risk_score
...
def hensman_training(nnet_model, type_nnet, epochs, dataset, optimiser, type_KL, num_samples, latent_dim, covar_module0,
                     ...
                     eps=1e-6, save_path=None, results_path=None, validation_dataset=None, generation_dataset=None,
                     prediction_dataset=None, test_dataset=None, gp_model=None, data_source_path=None, is_early_stopping=False, validation_criteria='nll',
                     dataset_type='HealthMNIST',num_workers=0):
    ...
    N = len(dataset)
    P = dataset.label_source.iloc[:,nnet_model.id_covariate].nunique()
    event_column = dataset.label_source.iloc[:, nnet_model.event_covariate]  # Directly access the column

    # subjects_per_batch = 100
    # Step 2: Filter rows where event_covariate is not -1
    filtered_dataset = dataset.label_source[event_column != -1]  # Correct filtering using column data
    id_column_name = dataset.label_source.columns[nnet_model.id_covariate]

    # Step 4: Calculate the number of unique values in the filtered DataFrame
    P_tr = filtered_dataset[id_column_name].nunique()

    P_val = validation_dataset.label_source.iloc[:,nnet_model.id_covariate].nunique()
    ...
    if varying_T:
        ...
        dataloader = HensmanDataLoader(dataset, batch_sampler=BatchSampler(SubjectSampler(dataset, P, T), batch_size, drop_last=False), num_workers=num_workers)
    ...
    S = prediction_dataset[0]['surv_covariates'].shape[0]

    net_train_loss_arr = np.empty((0, 1))
    recon_loss_arr = np.empty((0, 1))
    nll_loss_arr = np.empty((0, 1))
    kld_loss_arr = np.empty((0, 1))
    surv_loss_arr = np.empty((0, 1))
    train_c_index_arr = np.empty((0, 1))
    penalty_term_arr = np.empty((0, 1))
    validation_results = np.empty((0, 1))
    val_pred_loss_arr = np.empty((0, 1))
    val_c_index_arr = np.empty((0, 1))
    test_c_index_arr = np.empty((0, 1))
    val_recon_loss_arr = np.empty((0, 1))
    # best_val_cindex = 0
    ...
    if nnet_model.model_type == 'LVAE':
        print('Standard LVAE')
        # regression_ratio = 0.
    elif nnet_model.model_type == 'LVAE_survival':
        print('Joint Traininig-LVAE and Risk Regression')
        # regression_ratio = .9
    elif nnet_model.model_type == 'LVAE_survival_transformer':
        print('Joint Traininig-LVAE and Risk Regression with Transformer')
    ...
    for epoch in range(1, epochs + 1):
        ...
        id_index = 0
        iid_kld_sum = 0
        full_train_z_mu = torch.zeros(len(dataset), latent_dim, dtype=torch.double).to(device)
        full_labels = torch.zeros(len(dataset), Q, dtype=torch.double).to(device)
        full_surv_covariates = torch.zeros(len(dataset), S, dtype=torch.double).to(device)
        full_out = torch.zeros(P_tr, nnet_model.time_bin_num, dtype=torch.double).to(device)
        # full_src_mask = torch.zeros(P, nnet_model.max_length, dtype=torch.double).to(device)
        for batch_idx, sample_batched in enumerate(dataloader):
            ...
            full_train_z_mu[label_id] = mu
            full_labels[label_id] = train_x
            full_surv_covariates[label_id] = surv_covs
            [recon_loss, nll] = nnet_model.loss_function(recon_batch, data, mask)
            recon_loss = torch.sum(recon_loss)
            nll_loss = torch.sum(nll)  ##TODO[Changes]
            ...
            if nnet_model.model_type == 'LVAE':
                ...
                if nnet_model.transformer:
                    if 'long' in nnet_model.model_type:
                        survival_loss, out = nnet_model.risk_loss_transformer_long(structured_mu.double(), E, time_data,
                                                                              last_meas_time, last_meas,
                                                                              src_padding_mask.double())
                        full_out[id_index:id_index + out.shape[0], :] = out
                        id_index += out.shape[0]
                    else:
                    ...
                    if 'long' in nnet_model.model_type:
                        survival_loss, out = nnet_model.risk_loss_long(nnet_model.sample_latent(mu,log_var),
                                                                       E, time_data, last_meas_time, train_x, surv_covs)
                        if epoch == 1:
                            dot = make_dot(survival_loss, params=dict(list(nnet_model.named_parameters())))
                            dot.render('./model_visualization', format='png')  # Saves the graph as a PNG file

                        full_out[id_index:id_index + out.shape[0], :] = out
                        id_index += out.shape[0]
                    else:
                        survival_loss = nnet_model.risk_loss(nnet_model.sample_latent(mu,log_var), train_x, surv_covs)
                # Assuming 'tensor' is the tensor you want to track
                # hook = survival_loss.register_hook(backward_hook)

            if loss_function == 'nll':
                # net_loss = nll_loss + kld_loss + survival_loss * 10000
                surv_loss_sum += survival_loss.item()
                # net_loss = (nll_loss + kld_loss) * (1 - regression_ratio) + regression_ratio * survival_loss
                # Calculate the two parts of the loss
                part1 = nll_loss + kld_loss
                part2 = survival_loss

                # Calculate a ratio that would balance the two parts
                # Avoid division by zero
                # if part1.item() + part2.item() > 0:
                
                # else:
                #     balancing_ratio = regression_ratio  # keep the previous value if the sum is zero

                # Use a smoothing factor to avoid abrupt changes in the ratio
                smoothing_factor = 0.1  # this can be adjusted
                # regression_ratio = (1 - smoothing_factor) * regression_ratio + smoothing_factor * new_regression_ratio
                # regression_ratio = balancing_ratio

                # Now calculate the total loss with the dynamically adjusted regression_ratio
                # net_loss = part1 * (1 - regression_ratio) * (1-balancing_ratio) + regression_ratio * balancing_ratio * part2
                net_loss = part1 * (1 - nnet_model.regression_ratio)+ nnet_model.regression_ratio* part2
            ...
            net_loss_sum += net_loss.item()
            recon_loss_sum += recon_loss.item()
            nll_loss_sum += nll_loss.item()
            kld_loss_sum += kld_loss.item()

        print('Iter %d/%d - Duration: %.3f  - Loss: %.3f  - GP loss: %.3f  - NLL Loss: %.3f  - Recon Loss: %.3f -Survival Loss: %.5f' % (
            epoch, epochs, time.time()-start_time, net_loss_sum, kld_loss_sum, nll_loss_sum, recon_loss_sum, surv_loss_sum), flush=True)
        # penalty_term_arr = np.append(penalty_term_arr, 0.0)
        concordance_indx = calculate_risk_score(nnet_model, full_train_z_mu, full_labels, full_surv_covariates,
                                                save_path=os.path.join(save_path,'training_results'))
        train_c_index_arr = np.append(train_c_index_arr, concordance_indx)
        ...
        surv_loss_arr= np.append(surv_loss_arr, surv_loss_sum)

        # if epoch > 1:
        #     # log metrics to wandb
        #     wandb.log({"net_loss_sum": net_loss_sum,
        #                "recon_loss_sum": recon_loss_sum, "nll_loss_sum": nll_loss_sum,
        #                "kld_loss_sum": kld_loss_sum})
        if not epoch % 20:
            event_time = pd.DataFrame(full_labels).groupby(nnet_model.id_covariate).apply(
    lambda x: x.sort_values(by=nnet_model.time_covariate).tail(1)
)
            event_time = event_time[event_time[nnet_model.event_covariate]!=-1]
            event_time = event_time.iloc[:, [nnet_model.event_covariate, nnet_model.time_covariate]]

            plot_training_values(net_train_loss_arr, recon_loss_arr, nll_loss_arr, kld_loss_arr, surv_loss_arr, train_c_index_arr,
                                 results_path, full_out,event_time,epoch)
            torch.save(nnet_model.state_dict(), os.path.join(save_path, f'nnet_model_{epoch}.pth'))
            torch.save(gp_model.state_dict(), os.path.join(save_path, f'gp_model_{epoch}.pth'))
            torch.save(zt_list, os.path.join(save_path, f'zt_list_{epoch}.pth'))
            torch.save(m, os.path.join(save_path, f'm_{epoch}.pth'))
            torch.save(H, os.path.join(save_path, f'H_{epoch}.pth'))
            torch.save(full_train_z_mu, os.path.join(save_path, f'z_mu_train_{epoch}.pth'))

        if (not epoch % 3) and epoch != epochs:
            with torch.no_grad():
                nnet_model.eval()
                covar_module0.eval()
                covar_module1.eval()
                if validation_dataset is not None:
                    ...
                    for batch_idx, sample_batched in enumerate(dataloader):
                        ...
                        full_mu[label_id] = mu
                    val_pred_loss, _, val_c_index, val_recon_loss, val_gp_loss = validate(nnet_model, type_nnet, validation_dataset, type_KL,
                                                             num_samples, latent_dim, covar_module0, covar_module1,
                                                             likelihoods, zt_list, T, weight, full_mu, prediction_x,
                                                             id_covariate, loss_function, eps=1e-6,
                                                             subjects_per_batch=subjects_per_batch, num_workers=num_workers,
                                                                                          save_path=save_path)
                    # wandb.log({"val_pred_loss": val_pred_loss,
                    #            "val_c_index": val_c_index})
                    if validation_criteria == 'nll':
                        validation_results = np.append(validation_results, val_pred_loss)
                    elif validation_criteria == 'c_index':
                        validation_results = np.append(validation_results, -val_c_index)
                    elif validation_criteria == 'mse':
                        validation_results = np.append(validation_results, val_recon_loss)

                    val_pred_loss_arr = np.append(val_pred_loss_arr, val_pred_loss)
                    val_c_index_arr = np.append(val_c_index_arr, val_c_index)
                    val_recon_loss_arr = np.append(val_recon_loss_arr, val_recon_loss)

                    full_pred_mu = torch.zeros(len(prediction_dataset), latent_dim, dtype=torch.double).to(device)
                    ...
                    with torch.no_grad():
                        for batch_idx, sample_batched in enumerate(prediction_dataloader):
                            ...
                            mu, log_var = nnet_model.encode(data)
                            full_pred_mu[label_id] = mu
                        covar_module0.eval()
                        covar_module1.eval()
                        if type_KL == 'GPapprox' or type_KL == 'GPapprox_closed':
                            test_results = MSE_test_GPapprox(test_dataset,
                                                             data_source_path, type_nnet,
                                                             nnet_model, covar_module0, covar_module1, likelihoods,
                                                             results_path,
                                                             latent_dim, prediction_x,
                                                             full_pred_mu, zt_list, P, T, id_covariate, dataset_type,
                                                             varying_T,
                                                             is_early_stopping=False, num_workers=num_workers)
                            test_c_index_arr = np.append(test_c_index_arr, test_results[-2])


                    plot_validation_values([val_c_index_arr, test_c_index_arr, val_pred_loss_arr, val_recon_loss_arr],
                                           results_path, ['c_index', 'test_c_index', 'nll', 'mse'])
                    if (validation_results[-1] <= best_validation_score):
                        # best_val_cindex = val_c_index
                        # best_val_loss = val_pred_loss
                        best_validation_score = validation_results[-1]
                        best_epoch = epoch


                        # Stack the results
                        pred_results = np.array([val_pred_loss, val_recon_loss, val_c_index, val_gp_loss])
                        df_results = pd.DataFrame(pred_results[:, None], index=["NLL", "MSE", "C-Index", "GP Loss"])

                        # Save to CSV
                        df_results.to_csv(os.path.join(results_path, "best_validation_scores.csv"), index=True,
                                          header=False)
                        ...
                        with torch.no_grad():
                            ...
                            if type_KL == 'GPapprox' or type_KL == 'GPapprox_closed':
                                test_results = MSE_test_GPapprox(test_dataset,
                                                  ...
                                                  latent_dim, prediction_x,
                                                  full_pred_mu, zt_list, P, T, id_covariate, dataset_type, varying_T,
                                                  is_early_stopping=True, num_workers=num_workers)
                        ...
                        try:
                            ...
                            torch.save(full_mu, os.path.join(results_path, 'z_mu_train_best.pth'))
                            ...
                            if results_path and generation_dataset:
                                prediction_dataloader = DataLoader(prediction_dataset,
                                                                   batch_sampler=VaryingLengthBatchSampler(
                                                                       VaryingLengthSubjectSampler(prediction_dataset,
                                                                                                   id_covariate),
                                                                       subjects_per_batch), num_workers=num_workers)
                                full_pred_mu = torch.zeros(len(prediction_dataset), latent_dim, dtype=torch.double).to(
                                    device)
                                prediction_x = torch.zeros(len(prediction_dataset), Q, dtype=torch.double).to(device)
                                for batch_idx, sample_batched in enumerate(prediction_dataloader):
                                    label_id = sample_batched['idx']
                                ...
                                recon_complete_gen(generation_dataset, nnet_model, type_nnet,
                                                   ...
                                                   './data', prediction_x, full_pred_mu, -2,
        ...
        if not epoch % 50:
            if results_path and generation_dataset:
                ...
                recon_complete_gen(generation_dataset, nnet_model, type_nnet,
                                   results_path, covar_module0,
                                   covar_module1, likelihoods, latent_dim,
                                   './data', prediction_x, full_pred_mu, epoch,
                                   zt_list, P, T, id_covariate, varying_T)
    ...
    if varying_T:
        dataloader = HensmanDataLoader(dataset, batch_sampler=VaryingLengthBatchSampler(VaryingLengthSubjectSampler(dataset, id_covariate), P), num_workers=num_workers)
    else:
        dataloader = HensmanDataLoader(dataset, batch_sampler=BatchSampler(SubjectSampler(dataset, P, T), len(dataset), drop_last=False), num_workers=num_workers)

    for batch_idx, sample_batched in enumerate(dataloader):
        label_id = sample_batched['label']
        nnet_model.coxfitter = train_COX_on_latent(nnet_model, full_train_z_mu,
                                               label_id)

    if not is_early_stopping:
        torch.save(full_train_z_mu, os.path.join(results_path, 'z_mu_train.pth'))
    else:
        torch.save(full_train_z_mu, os.path.join(results_path, 'z_mu_train_best.pth'))
...
def minibatch_training(nnet_model, type_nnet, epochs, dataset, optimiser, type_KL, num_samples, latent_dim, 
                       ...
                       generation_dataset=None, prediction_dataset=None, num_workers=0):
...
#            param_list.append(zt_list[i])
        ...
        if (not epoch % 100) and epoch != epochs:
            if validation_dataset is not None:
                validate(nnet_model, type_nnet, validation_dataset, type_KL, num_samples, latent_dim,
                         covar_module0, covar_module1, likelihoods, zt_list, T, weight, full_mu, train_x,
                         id_covariate, loss_function, eps=1e-6, num_workers=num_workers, save_path=save_path)
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()

            if results_path and generation_dataset:
                prediction_dataloader = DataLoader(prediction_dataset, batch_size=1000, shuffle=False, num_workers=num_workers)
